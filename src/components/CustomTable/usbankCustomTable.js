...
function CustomTable({
  ...
  selectableDataAppRej,
  selectedAppRejPayment,
  handleSelectAllClickPayment,
  handleRowItemClickPayment,
  user,
}) {
  ...
  const statusInfo =
    ...
    statusList.map((item) => {
      return (
        <Box display="flex" key={item.StatusID}>
        ...
        >
          <TableHead>
            <StyledTableRow>
              ...
              {isApproveEnabled && isRejectEnabled ? (
                <StyledTableCell align="center">
                  <span>
                    <Checkbox
                      checked={
                        selectableDataAppRej?.length !== 0 &&
                        selectedAppRejPayment.length ===
                          selectableDataAppRej.length
                      }
                      color="primary"
                      indeterminate={
                        selectedAppRejPayment.length > 0 &&
                        (selectableDataAppRej.length > 10
                          ? selectedAppRejPayment.length < 10
                          : selectedAppRejPayment.length <
                            selectableDataAppRej.length)
                      }
                      onChange={(event) => handleSelectAllClickPayment(event)}
                      icon={<CheckBoxOutlineBlankIcon fontSize="small" />}
                      checkedIcon={<CheckBoxIcon fontSize="small" />}
                      indeterminateIcon={
                        <IndeterminateCheckBoxIcon fontSize="small" />
              ...
              <StyledTableCell align="left">
                <span className={customClasses.header}>
                  ...
                  <input
                    ...
                    disabled={isLoading}
                    placeholder={t(
                      "componentData.customTable.PaymentReference"
                    )}
                    id="paymentRef"
              ...
              <StyledTableCell align="left">
                <span className={customClasses.header}>
                  <span className={customClasses.headerLabel}>
                    {t("componentData.customTable.PayeeType")}
                  </span>
                  <FormControl variant="outlined">
                    <Select
                      id="PayeeTypeId"
                      name="PayeeTypeId"
                      value={dataFilterParamsObj?.PayeeTypeId ?? 0}
                      onChange={onChangeInput}
                      disabled={isLoading}
                      style={{
                        background: "#fff",
                    ...
                    >
                      <MenuItem value={0}>
                        {t("componentData.customTable.All")}
                      </MenuItem>
                ...
                <span className={customClasses.header}>
                  ...
                  <FormControl variant="outlined">
                    ...
                    >
                      ...
                      <MenuItem id={8} key={8} value={8}>
                        {"Blank Payment"}
                      </MenuItem>
                      {Object.keys(apiPaymentTypesList).map((key, index) => {
                        if (Object.keys(payTypeList).length > 0) {
              ...
              <StyledTableCell align="left">
                <span className={customClasses.header}>
                  <span
                    ...
                    style={{ width: "130px" }}
                  >
                    {t("componentData.customTable.substatus")}
                  </span>

                  <FormControl variant="outlined">
                    <input
                      className={customClasses.headerInput}
                      placeholder={t("componentData.customTable.substatus")}
                      disabled={isLoading}
                      id="PaymentSubStatus"
                      value={dataFilterParamsObj.PaymentSubStatus || ""}
                      onChange={onChangeInput}
                    />
                  </FormControl>
                </span>
                ...
                <span className={customClasses.header}>
                  ...
                  <FormControl variant="outlined">
                    <Select
                      id="statusIDs"
                      name="statusIDs"
                      value={dataFilterParamsObj?.statusIDs ?? "all"}
                      onChange={onChangeInput}
                      disabled={isLoading}
                      style={{
                        background: "#fff",
                  ...
                  <span className={customClasses.parentSpan}>
                    <FormControl variant="outlined">
                      <Select
                        id="AmountFilterBy"
                        name="AmountFilterBy"
                        value={dataFilterParamsObj?.AmountFilterBy ?? "0"}
                        onChange={(e) => {
                          onChangeInput(e);
                        }}
                        disabled={isLoading}
              ...
              <StyledTableCell align="left">
                <span className={customClasses.header}>
                  <span className={customClasses.headerLabel}>
                    {t("componentData.customTable.Lastupdatedon")}
                  </span>
                  <NumberFormat
                    className={customClasses.headerInput}
                    format="##/##/####"
                    ...
                    disabled={isLoading}
                    placeholder={t("componentData.customTable.dateFormate")}
                    value={dataFilterParamsObj.LastUpdatedOn || ""}
                    onChange={(e) => {
                      const date = e.target.value;
                      if (isValidDate(date) || date === "") {
                        onChangeInput(e);
              ...
              <StyledTableCell align="left">
                <span className={customClasses.header}>
                  <span className={customClasses.headerLabel}>
                    {t("componentData.customTable.Createdon")}
                  </span>
                  <NumberFormat
                    className={customClasses.headerInput}
                    format="##/##/####"
                    ...
                    disabled={isLoading}
                    placeholder={t("componentData.customTable.dateFormate")}
                    value={dataFilterParamsObj.CreatedOn || ""}
                    onChange={(e) => {
                      const date = e.target.value;
                      if (isValidDate(date) || date === "") {
                        onChangeInput(e);
              ...
              {rows && rows.length > 0
                ? rowsPerPage > 0 &&
                  rows.map((row) => {
                    ...
                    const isSelectedAppRej =
                      selectedAppRejPayment
                        .map((item) => item.PaymentID)
                        .indexOf(row.PaymentID) !== -1;
                    return (
                      <StyledTableRow
                      ...
                      >
                        ...
                        {isApproveEnabled && isRejectEnabled ? (
                          ...
                          >
                            {row.ReturnStatusID ===
                            PaymentPendingApproval[0] ? (
                              <Checkbox
                                onChange={(event) =>
                                  handleRowItemClickPayment(event, row)
                                }
                                icon={
                                  <CheckBoxOutlineBlankIcon fontSize="small" />
                                }
                                checkedIcon={<CheckBoxIcon fontSize="small" />}
                                checked={isSelectedAppRej}
                                color="primary"
                                disabled={
                                  row?.CreatedByUserID ===

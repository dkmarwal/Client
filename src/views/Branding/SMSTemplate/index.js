...
import { fetchSMSTemplates } from '~/redux/helpers/B2C/branding';
import { updateSmsNotification } from '~/redux/actions/branding';
...
class SMSTemplate extends React.Component {
  ...
  getData() {
    ...
    fetchSMSTemplates(clientId).then((response) => {
      if (response.error) {
  ...
  render() {
    ...
    const isBrandingSMSEditEnabled =
      (user.userRoles &&
        user.userRoles.includes(accessRights['BRANDING_SMS_TEMPLATE_EDIT'])) ||
      false;

    const isBrandingSMSToggleButtonEnabled =
      (user.userRoles &&
        user.userRoles.includes(
          accessRights['BRANDING_SMS_TEMPLATE_TOGGLE_VIEW']
        )) ||
      false;
                  ...
                  <Table>
                    ...
                    >
                      <TableRow>
                       ...
                       <StyledTableCell>
                          {isBrandingSMSToggleButtonEnabled && isPayeeChoicePortal
                          ...
                          <TableBody>
                            <Fragment key={index}>
                              <StyledTableRow>
                                ...
                                <StyledTableCell>
                                  {isBrandingSMSToggleButtonEnabled && (
                                    <Switch

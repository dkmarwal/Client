...
export const fetchSuppliersFilterList = async (key, clientId, data, limit, page, payerTypeId) => {
  let offset = (page * limit);
  let urlLink = '';
  if(payerTypeId == PayerTypes.CARDS){
    urlLink = `${config.apiBase.payeeService}/cc-payees?supplierId=${encodeURIComponent(data.id)}&payeeName=${encodeURIComponent(data.name)}&payeeType=${data.payeeType}&spend=${data.committedSpend}&expectedResult=${data.expectedResult}&fromDate=${data.fromDate}&toDate=${data.toDate}&campaignVendorId=${data.onBoardSource.toString().trim()}&ccCampaignId=${data.selectedCampaign}&currency=${data.selectedCurrency}&limit=${limit}&offset=${offset}`;
  }
  else{
    urlLink = key === "" ? `${config.apiBase.payeeService}/payees?clientId=${clientId}&implementationProgram=${data.programList}&remitToId=${encodeURIComponent(data.id)}&state=${data.location}&companyName=${encodeURIComponent(data.name)}&paymentMethod=${data.paymentList}&status=${data.status}&limit=${limit}&offset=${offset}&sort=${data.sort}&sortType=${data.sortType}` :
...
export const fetchSuppliersGraphData = async (campId=null) => {
  const urlLink = campId===null ?`${config.apiBase.payeeService}/cc-payees-status`
  :`${config.apiBase.payeeService}/cc-payees-status?campaignId=${campId}`;
  try{
    const accessToken = await getAccessToken();
    const response = await axios({
      url: urlLink,
...
export const fetchExpectedResultFilterList = async () => {
  try {
    const accessToken = await getAccessToken();
    const response = await axios({
      url: `${config.apiBase.payeeService}/cc-getExpectedResultInfo`,
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,
...
export const fetchCampaignVendorList = async () => {
  try {
    const accessToken = await getAccessToken();
    const response = await axios({
      url: `${config.apiBase.payeeService}/getCampaignVendorList`,
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,
...
export const fetchCampaignList = async (selectedVendorId) => {
  try {
    const accessToken = await getAccessToken();
    const response = await axios({
      url: `${config.apiBase.payeeService}/getCampaignsByVendor?campaignVendorId=${selectedVendorId.toString().trim()}`,
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,
...
export const updatePayeeDetails = async (data) => {
  try {
    const accessToken = await getAccessToken();
    const response = await axios({
      url: `${config.apiBase.payeeService}/payer-payees`,
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,
...
export const fetchSuppliersCount = async () => {
  try {
    const accessToken = await getAccessToken();
    const response = await axios({
      url: `${config.apiBase.payeeService}/payees/onboarding/count`,
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,
...
export const getNpiIdList = async (payeeId, actPayeeId) => {
  try {
    const accessToken = await getAccessToken();
    const response = await axios({
      url: `${config.apiBase.payeeService}/getNpiIds`,
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,
      ...
      data: JSON.stringify({
        ...
        actPayeeId: actPayeeId || undefined
      }),
    });
    const responseBody = await response.data;
    return responseBody;
...
// Commercial card
export const getPayeeDetails = async (payeeRegInfoId, portalProfileId) => {
  try {
    const accessToken = await getAccessToken();
    const response = await axios({
      url: `${config.apiBase.masterCardService}/1/payeeDetails?PayeeID=${payeeRegInfoId}&ClientID=${portalProfileId}`,
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,
...
export const deactivatePayee = async (data) => {
  try {
    const accessToken = await getAccessToken();
    const response = await axios({
      url: `${config.apiBase.payeeService}/deactivatePayeeCC`,
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,
...
export const updatePayeeRisk = async (data) => {
  try {
    const accessToken = await getAccessToken();
    const response = await axios({
      url: `${config.apiBase.payeeService}/notConsiderPayeeRisk`,
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,
...
export const getPayeeRiskDetails = async (id) => {
  try {
    const accessToken = await getAccessToken();
    const response = await axios({
      url: `${config.apiBase.payeeService}/getNotConsiderPayeeRisk?payeeRegInfoId=${id}`,
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,

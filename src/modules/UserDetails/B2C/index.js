...
class UserDetails extends Component {
  render() {
    ...
    const {parentDetails} = client
    const tabsDisabled = isPayeeChoicePortal ? true : false
    if(parentDetails && parentDetails.rows?.length){
      isSSO = parentDetails.rows[0].isSSO
    }
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={6} sm={6}>
            <CheckboxGroup
              ...
              selectedOption={signOnType}
              disabled={tabsDisabled}
            />
          </Grid>
          <Grid item xs={6} sm={6}></Grid>

          {signOnType === "StandAlone" && (!isPayeeChoicePortal || (isPayeeChoicePortal && !isSSO)) ? (
            <>
              <Grid item xs={6} sm={6}>
                <input
                  type="text"
          ...
          {signOnType === "SSO"  && (!isPayeeChoicePortal || (isPayeeChoicePortal && isSSO)) ? (
            <Grid item xs={6} sm={6}>
              <TextField
                required
                fullWidth={true}
...
export default withTranslation()(
  connect((state) => ({ ...state.client,...state.user }))(withStyles(styles)(UserDetails))
);

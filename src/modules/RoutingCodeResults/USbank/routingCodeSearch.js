  ...
  handleValidation = () => {
    ...
    if (routingSearchMode === 1) {
      if (!routingSearchVal || isNaN(routingSearchVal)) {
        error.routingSearchVal = t('componentData.routingCodeResult.error.routingNumber');
        isValid = false;
      }
    } else {
      if (!bankName || !bankName?.trim().length) {
        error.bankName = t('componentData.routingCodeResult.error.bankName');
        isValid = false;
      }
      if (!bankState || !bankState?.trim().length) {
        error.bankState = t('componentData.routingCodeResult.error.bankState');
        isValid = false;
      }
      if (!bankCity || !bankCity?.name?.trim().length) {
        error.bankCity = t('componentData.routingCodeResult.error.bankCity');
        isValid = false;
      } else if (bankCity?.name?.trim().length > 30) {
        error.bankCity = t('componentData.routingCodeResult.error.bankCityLength');
        isValid = false;
      }
    }
    this.setState({
        ...
        {!routingCodeResults ? (
          ...
          >
            <Grid item>
              ...
              {t('componentData.routingCodeResult.heading.searchBank')}
              </Typography>
            </Grid>
            <Grid item>
              <RadioGroup
              ...
              >
                <FormControlLabel
                  ...
                  label={t('componentData.routingCodeResult.heading.byRoutingNumber')}
                />
              </RadioGroup>
            </Grid>
            <Grid item className={classes.textFieldItem}>
              <TextField
                ...
                label={t('componentData.routingCodeResult.label.routingNumber')}
                required
            ...
            >
              ...
              <Grid item xs={2} lg={2} className={classes.orText}>
                {t('componentData.routingCodeResult.label.or')}
              </Grid>
              <Grid item xs={5} lg={5}>
                <Divider className={classes.dividerLine} />
              </Grid>
            ...
            <Grid item>
              ...
              >
                <FormControlLabel
                  ...
                  classes={{ label: classes.routingLabel }}
                  label={t('componentData.routingCodeResult.heading.byBankName')}
                />
              </RadioGroup>
            </Grid>
            <Grid item className={classes.textFieldItem}>
              <TextField
                className={classes.routingNumberTextField}
                label={t('componentData.routingCodeResult.label.bankName')}
                required
                value={bankName ?? ''}
                variant="outlined"
                name="bankName"
            ...
            <Grid item container spacing={2}>
              <Grid item xs={12} lg={6} style={{marginTop: '16px', marginBottom: '8px'}}>
                <StateIso
                  name="bankState"
                  label={t('componentData.routingCodeResult.label.bankState')}
              ...
              <Grid item xs={12} lg={6} className={classes.routingNumberTextFieldMob}>
                <Autocomplete
                  ...
                  disabled={routingSearchMode === 1}
                  value={bankCity ?? ''}
                  noOptionsText={t('componentData.routingCodeResult.text.noOptions')}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      inputProps={{
                      ...
                      label={t('componentData.routingCodeResult.label.bankCity')}
                      variant="outlined"
                      name="bankCity"
                      color="secondary"
            ...
            >
              <Grid item xs={5} sm={4} md={4} lg={5}>
                <Button
                  ...
                  fullWidth
                >
                  {t('componentData.routingCodeResult.button.cancel')}
                </Button>
              </Grid>
              <Grid item xs={5} sm={4} md={4} lg={5}>
                <Button
                  ...
                  fullWidth
                >
                  {t('componentData.routingCodeResult.button.search')}
                </Button>
              </Grid>
            </Grid>
          </Box>
...
export default withTranslation()(
  connect((state) => ({  ...state.csc, ...state.payment  }))(withStyles(styles)(RoutingCodeSearch))
);

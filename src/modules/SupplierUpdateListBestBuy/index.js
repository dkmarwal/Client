...
import ReviewPayeeUpdateFilters from "~/modules/ReviewPayeeUpdateFiltersBestBuy";
...
const SupplierUpdateListBestBuy = (props) => {
  const { bestBuySupplierUpdateList } = useSelector(
  ...
  useEffect(() => {
    const selectedCardIndex = _.findIndex(
      ...
      (item) => item.payerReviewUpdateId === props.selectedCard
    );
    const newPage =
      selectedCardIndex !== -1
  ...
  useEffect(() => {
    bestBuySupplierUpdateList.length > 0 && handleChangePage(null, 0);
  }, [rowsPerPage]);
  ...
  const handleChangePage = (e, newPage) => {
    ...
    handleClick(
      bestBuySupplierUpdateList[nextIndex].payeeId,
      bestBuySupplierUpdateList[nextIndex].payerReviewUpdateId,
      bestBuySupplierUpdateList[nextIndex].subjectType,
      bestBuySupplierUpdateList[nextIndex].action
    );
  };
  ...
  const handleStatusClick = (item, index, type) => {
    setStatus(
      status.map((list, i) => {
        return index === i
          ? {
  ...
  const handleClick = async (
    ...
    props.onClickHandler(payeeId, payerReviewUpdateId, subjectType, action);
  };
  ...
  const getTypeOfStatus = () =>
    status.reduce(
      (a, o) => (o.selected && a.push({ ...o, type: "statusSearch" }), a),
      []
    );
  ...
  const setSelectedFilters = () => {
    ...
    obj = status
      ? [...obj, ...getTypeOfStatus()]
      : removeSelectedFilter("statusSearch", obj);
  ...
  const handleOnDelete = (filter) => {
    switch (filter.type) {
      ...
      case "actionTypes":
        ...
        break;
      case "statusSearch":
        handleStatusClick(filter, filter.id);
        break;
      default:
    }
    setClicked(true);
  ...
  const selectedCardIndex = _.findIndex(
    ...
    (supplierUpdateListItem) =>
      supplierUpdateListItem.payerReviewUpdateId === props.selectedCard
  );
  if (props.isLoading) {
    return (
  ...
  return (
    <>
      {bestBuySupplierUpdateList && bestBuySupplierUpdateList.length === 0 && (
        <Grid container>
          <Grid
            container
            direction="row"
              ...
              <Table>
                <TableRow>
                  <TablePagination
                    ...
                    component="div"
                    count={bestBuySupplierUpdateList.length || 0}
        ...
        <Box width="38%">
          <>
            <Box>
              ...
              )} ${bestBuySupplierUpdateList.length} ${t(
                ...
                (
                  ...
                  const content = (
                    <Typography variant="h5">
                      {renderContent(action, subjectType)}
                    </Typography>
                  );
                  const footer = (
                    <>
                      ...
                      {recordStatusId === 1 && (
                        ...
                        >
                          {`${t(
                            "componentData.supplierUdateList.AcceptedBy"
                          )} ${updatedByUserName || "--"} ${t(
                            "componentData.supplierUdateList.at"
                          )} ${updatedAt || "--"}`}
                        </Typography>
                      ...
                      {recordStatusId === 11 && (
                        ...
                        >
                          {` ${t(
                            "componentData.supplierUdateList.RejectedBy"
                          )} ${updatedByUserName || "--"} ${t(
                            "componentData.supplierUdateList.at"
                          )} ${updatedAt || "--"}`}
                        </Typography>
                    ...
                    <>
                      <Box m={1} key={index}>
                        <CustomCard
                          ...
                          checked={checked}
                          payeeActionTypeId={payerReviewUpdateId}
...
export default withTranslation()(withRouter(SupplierUpdateListBestBuy));

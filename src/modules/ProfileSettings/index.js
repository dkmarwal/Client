...
import OnboardingCompanyDetails from "~/modules/OnboardingCompanyDetails";
...
class ProfileSettings extends Component {
  ...
  prefilledCompanyInfoData = (data, isCompanyData) => {
    ...
    companyInfoObj.country.value = isCompanyData
      ...
      : data.clientLocations && data.clientLocations.length > 0
        ...
        : "US";
    if (isCompanyData) {
      if (data.taxIdIsSSN === 2) {
        companyInfoObj.taxidOrSSN.value = data.identificationType ? data.identificationType : t("componentData.editCompanyView.IdentificationNumber");
      }
      else {
  ...
  saveCompanyInfoData = () => {
    this.setState(
      ...
      () => {
        ...
        this.props
          .dispatch(updateCompanyData(clientId, data))
          .then((response) => {
            if (!response) {
              throw this.props.client.error;
            }
  ...
  saveAdminCredentials = () => {
    ...
    this.props
      .dispatch(createUser(2, clientId, data))
      .then((response) => {
        if (!response) {
          throw this.props.client.error;
        }
  ...
  getParentCompanyData = (id) => {
    this.props.dispatch(fetchParentCompanyData(id)).then((response) => {
      if (!response) {
        this.setState({
          parentCompanyData: {},
        });
  ...
  validateData = () => {
    ...
    for (const [key, value] of Object.entries(companyInfoObj)) {
      ...
      if (
        key === "phone" && companyInfoObj[key].value.toString().trim().length !== 10
      ) {
        valid = false;
  ...
  onBlurValidate = (event) => {
    ...
    switch (name) {
      ...
      case "phone":
        {
          if (
            companyInfoObj.phone.value.length !== 10
          ) {
            errorText["phone"] = t("componentData.profileSettings.phoneLen");
          }
        }
      ...
      case "website":
        {
          if (value.length !== 0) {
            ...
            if (!re.test(value)) {
              errorText["website"] = t(
                "componentData.profileSettings.enterWeb"
              );
            }
          }
        }
  ...
  validateUserData = () => {
    ...
    for (const [key, value] of Object.entries(userInfoObj)) {
      ...
      if (
        ...
        valid = false;
        errorText["confirm_pass"] = t("componentData.profileSettings.PasswordDoesNotMatch");
      }
      if (key === "user_email") {
  ...
  onUserBlurValidate = (event) => {
    ...
    switch (name) {
      ...
      case "user_name":
        {
          if (!isSSO && value.length === 0) {
            errorText["user_name"] = t(
              "componentData.profileSettings.userName"
            );
          }
        }
        break;
      case "user_pass":
        {
          if (
            !isSSO &&
            (value.length === 0 || value.match(pwdRegex) === null)
          ) {
            errorText["user_pass"] = t(
              "componentData.profileSettings.Password"
            );
          }
        }
        break;
      ...
      case "user_phone":
        {
          if (value.length !== 10) {
            errorText["user_phone"] = t(
              "componentData.profileSettings.phoneLen"
            );
          }
        }
        break;
...
export default withTranslation()(
  connect((state) => ({ ...state.client }))(ProfileSettings)
);

...
class ImportPaymentFiles extends Component {
  ...
  getActionTypeList = async () => {
    ...
    const res = await fetchActionTypeList(portalProfileId, payerTypeId, appType);
  ...
  getFileStatusList = async () => {
    ...
    if (res.error) {
      ...
      let data = [];
      res.data && res.data.map(function (detail) {
        data = [
          ...data,
  ...
  handleChangeRowsPerPage = (event) => {
    ...
    const data = {
      ...
      pageNumber: isPayeeChoicePortal?!filterQuery.rowCount===event.target.value?1:filterQuery.pageNumber:filterQuery.pageNumber,
      BusinessType: appType,
    };
    this.setState(
      {
  ...
  render() {
    ...
    const appType = user.userData.appType
      ...
      : entityType.B2B;
    const payerTypeId = user.userData && user.userData.payerTypeId ? user.userData.payerTypeId : PayerTypes.PMTX;
    const columns =
      appType === 1
        ? [
          ...
          { id: 'FileUploaded', label: 'Uploaded On' },
    ...
    return (
      <Box mx={6} my={2}>
        <Grid container item xs={12} md={12} justify="flex-end">
          <Box mt={'-42px'}>
            {isMyFileUploadEnabled && (appType !== 2 || user?.isPayeeChoicePortal) && (
              <Button
                style={this.props.i18n.language === 'fr' ? { width: 330 } : {}}
                variant="contained"
        ...
        {openDialogue && (
          <ImportFileUpload open={true} onCancel={this.goToFilesList} />
        )}
            ...
            >
              <Box display="flex" justifyContent="flex-end">
                <Box mt={1} mr={1} className="button-container">
                  ...
                  <Box>
                    {isMyFileApproveEnabled && this.shouldActionButtonEnable('approve') && (
                      <Button
                        color={
                  ...
                  <Box>
                    {isMyFileRejectEnabled && this.shouldActionButtonEnable('reject') && (
                      <Button
                        color={
                  ...
                  <Box>
                    {isMyFileRecalcEnabled && appType !== 2 && this.shouldActionButtonEnable('recalc') && (
                      <Button
                        color={
                        ...
                        disabled={
                          this.shouldActionButtonEnable('recalc') ? false : true
                        }
                      >
                      ...
                      >
                        ...
                        {appType === 1 ? (
                          ...
                          >
                            ...
                            {t('componentData.importPaymentFiles.ID')}{' '}
                            {row.FileID}
                          </TableCell>
                        ) : (
                          <>
